...
def run_training_pipeline():
    ...
    data, feature_cols, scaler = load_and_preprocess_data(
    ...
    print("[INFO] Exporting test_traffic.csv for FastAPI dashboard...")
    ...
    plot_rf_results(
        ...
        rf_model,
        feature_cols,
        save_path='rf_evaluation.png'
    )

    # -----------------------------
    # 4. DQN TRAINING (RL AGENT)
    # -----------------------------
    print("\n--- Phase 4: DQN Training ---")

    train_env = Monitor(
        NetworkIDSEnv(train_data, feature_cols, rf_oracle=rf_model)
    )

    eval_env = Monitor(
        NetworkIDSEnv(eval_data, feature_cols, rf_oracle=rf_model)
    )
    ...
    rewards, actions, infos = evaluate_dqn(dqn_agent, eval_env)

    plot_dqn_results(
        rewards,
        actions,
        save_path='dqn_evaluation.png'
    )
